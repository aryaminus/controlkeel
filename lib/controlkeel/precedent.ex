defmodule ControlKeel.Precedent do
  ...
  should survive session boundaries.
  """
  def for_rule_ids(rule_ids, opts \\ []) when is_list(rule_ids) do
    ...
    |> Enum.take(opts[:limit] || @max_precedent_entries)
  end
  ...
  defp entry_session_id(entry) do
    Map.get(entry, :session_id) || get_in(Map.get(entry, :metadata, %{}), ["session_id"])
  end
  ...
  current session's domain and active finding rule_ids.
  """
