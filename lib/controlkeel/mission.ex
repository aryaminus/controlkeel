defmodule ControlKeel.Mission do
  ...
  defdelegate list_sessions, to: SessionOps
  defdelegate get_session(id), to: SessionOps
  defdelegate get_session!(id), to: SessionOps
  defdelegate get_session_by_proxy_token(token), to: SessionOps
  defdelegate get_session_with_workspace(id), to: SessionOps
  defdelegate create_session(attrs), to: SessionOps
  defdelegate update_session(session, attrs), to: SessionOps
  defdelegate attach_session_runtime_context(session_or_id, context), to: SessionOps
  defdelegate delete_session(session), to: SessionOps
  defdelegate list_workspaces, to: SessionOps
  defdelegate get_workspace!(id), to: SessionOps
  defdelegate list_workspaces_for_org(org_id), to: SessionOps
  defdelegate list_sessions_for_workspace(workspace_id), to: SessionOps
  defdelegate create_workspace(attrs), to: SessionOps
  defdelegate update_workspace(workspace, attrs), to: SessionOps
  defdelegate get_workspace_by_slug(slug), to: SessionOps
  defdelegate get_task(id), to: SessionOps
  defdelegate get_task!(id), to: SessionOps
  defdelegate get_task_by_external_id(external_id), to: SessionOps
  defdelegate project_name_taken?(name), to: SessionOps

  defdelegate bind_github_repo(workspace_id, owner, repo, opts \\ []), to: SessionOps
  defdelegate unbind_github_repo(workspace_id, owner, repo), to: SessionOps
  defdelegate list_github_repos(workspace_id), to: SessionOps
  ...
  def generate_proof_bundle(%Task{} = task) do
    RepoRetry.transaction_with_busy_retry(fn ->
  ...
  @doc false
  def record_brief_memory(%Session{workspace_id: nil}), do: :ok

  def record_brief_memory(%Session{} = session) do
  ...
  @doc false
  def merge_runtime_context(metadata, runtime_context) when is_map(runtime_context) do
