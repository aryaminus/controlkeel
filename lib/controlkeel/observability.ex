defmodule ControlKeel.Observability do
  ...
  def observability_benchmark_history(opts \\ []) do
    ...
    coverage = %{
      ...
      # ⚡ Bolt: Use Enum.count/2 instead of length(Enum.filter/2) to avoid intermediate list allocation. ~50% faster, O(1) memory overhead.
  ...
  defp cost_totals(invocations) do
    %{
      ...
      # ⚡ Bolt: Use MapSet.new/2 + MapSet.size/1 instead of Enum.map |> Enum.uniq |> length to avoid allocating two intermediate lists. Memory efficient and avoids O(N) length walk.
  ...
  defp cost_groups(invocations, by) do
    ...
    |> Enum.map(fn {name, group} ->
      %{
        ...
        # ⚡ Bolt: MapSet.size(MapSet.new) replaces Enum.map |> Enum.uniq |> length to drop intermediate list building overhead.
  ...
  defp comparison_groups(invocations, by) do
    ...
    |> Enum.map(fn {name, group} ->
      ...
      %{
        ...
        # ⚡ Bolt: O(1) MapSet count replaces O(N) length walk over 2 allocated lists.
        sessions: group |> MapSet.new(& &1.session_id) |> MapSet.size(),
