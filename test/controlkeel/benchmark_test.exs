defmodule ControlKeel.BenchmarkTest do
  ...
  describe "exports benchmark runs as an EvalPort/OpenEval bundle" do
    ...
    test "keeps a real result while excluding a sibling awaiting_import result in the same run",
         %{tmp_dir: tmp_dir} do
      write_benchmark_subjects!(tmp_dir, [
        %{
          "id" => "manual_subject",
      ...
      {:ok, run} =
        Benchmark.run_suite(
          %{
            "suite" => "vibe_failures_v1",
            "subjects" => "controlkeel_validate,manual_subject",
            "baseline_subject" => "controlkeel_validate",
            "scenario_slugs" => "hardcoded_api_key_python_webhook"
          },
          tmp_dir
      ...
      statuses = Map.new(run.results, &{&1.subject, &1.status})
      assert statuses["controlkeel_validate"] == "completed"
      assert statuses["manual_subject"] == "awaiting_import"

      assert {:ok, output} = Benchmark.export_run(run.id, "openeval")
      assert {:ok, bundle} = Jason.decode(output)

      # Regression guard alongside the exclusion test above: filtering out an
      # awaiting_import result must not touch its sibling in the same run.
      # A naive "drop by index" or "bail out of the whole mapping if any
      # result is pending" implementation would silently lose the real,
      # completed result too -- this pins the filter to per-result status,
      # not run-wide state. See aryaminus/controlkeel#153 review.
      results = bundle["result_set"]["results"]
      assert length(results) == 1

      [only_result] = results
      assert only_result["metadata"]["controlkeel_subject"] == "controlkeel_validate"
      assert only_result["passed"] == true

      refute Enum.any?(results, &(&1["metadata"]["controlkeel_subject"] == "manual_subject"))
    end
  end
