...
The contract also declares `review_risk` (`standard`, `high`, or `critical`) and required review personas. The target packet records the worker agent, provider, display model, and canonical provider-issued model ID. Review annotations record the same identity fields plus covered personas. Every reviewer must be a different agent; high-risk work needs at least one different canonical model ID; critical work permits no matching canonical model ID. Missing required personas block promotion. Do not derive canonical identity from a deployment alias or display label.
