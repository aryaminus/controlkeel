...
The contract also declares `review_risk` (`standard`, `high`, or `critical`) and required review personas. The target packet records the worker agent, provider, and model. Review annotations record reviewer identities and covered personas. Every reviewer must be a different agent; high-risk work needs at least one different model; critical work permits no same-model reviewer. Missing required personas block promotion.
